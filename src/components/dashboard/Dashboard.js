...
function CloudButton({ id, icon, title, desc, mode, floatClass, tourClass, backgroundImage, onAction }) {
  ...
  return (
    <div className={`cloud-btn ${floatClass} ${tourClass}`} onClick={handleClick} style={{ backgroundImage }}>
      <span className='cloud-icon' style={{ display: 'flex', justifyContent: 'center', alignItems: 'center', position: 'relative', zIndex: 1 }}>
        {icon}
      </span>
      <div className='cloud-title' style={{ position: 'relative', zIndex: 1 }}>
        {title}
      </div>
      <div className='cloud-desc' style={{ position: 'relative', zIndex: 1 }}>
        {desc}
      </div>
    </div>
  )
...
export default function Dashboard({ onSelectMode }) {
  return (
    <div className='clouds-dashboard'>
      ...
      <CloudButton
        id='cloud-6'
        mode='create-cv-questions'
        title='CV con preguntas'
        desc='Guía (Scratch)'
        icon={<HelpCircle size={36} color='#1e3a8a' />}
        floatClass='float-2'
        tourClass='tour-cv-questions'
        backgroundImage="url('/nube.png')"
        onAction={onSelectMode}
      />
    </div>
